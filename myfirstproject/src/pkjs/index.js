...
function getWeatherIconFromCode(code, conditionText, isDay) {
  // Sunny/Clear (icon 0)
  if (code === 1000) {
    // If it's night, return moon icon (8)
    if (isDay === 0) return 8;
    return 0;
  }
  ...
  xhr.onload = function() {
    if (xhr.readyState === 4 && xhr.status === 200) {
      try {
        ...
        var iconCode = getWeatherIconFromCode(conditionCode, condition, isDay);
